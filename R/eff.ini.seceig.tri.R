...
eff.ini.seceig.tri = function(a, b, xi = 1, digit.thresh = 6) {
    ...
    eta1 = max(1/(mu[1:N] * b[1:N] * diff(v0_tilde)) * etapartsum[2:(N + 1)])
    
    zstart = xi * 1/eta1 + (1 - xi) * sum(v0_bar * (-Q %*% v0_tilde) * mu)/(sum(v0_bar^2 * 
        mu))
    
    ray = ray.quot.seceig.tri(Q = Q, mu = mu, v0_tilde = v0_bar, zstart = zstart, 
